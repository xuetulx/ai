...
/**
 * dsh-plugin-hardbound / core
 * ---------------------------------------------------------
 * DSH 硬边界规则注入插件核心逻辑（零外部依赖）。
 *
 ...
 *
 * 本插件职责：把插件内 rules/ 目录下的硬边界规则合并为 $DSH_HOME/AGENTS.md。
 * 特性：
 *   - 多插件共存协议：识别任意 `dsh-plugin-*` 管理块，只重建自己的块，
 *     其他插件块原样保留，多个规则插件可同时挂载互不覆盖；
 *   - 保留用户区：AGENTS.md 中"所有插件管理块之外"的内容原样保留；
 *   - 首次接管自动备份：AGENTS.md.pre-plugin-hardbound.bak（幂等，仅一次）；
 *   - 幂等同步：内容哈希无变化时不写文件；
...
const PLUGIN_ID = 'dsh-plugin-hardbound';
...
const BACKUP_NAME = 'AGENTS.md.pre-plugin-hardbound.bak';
...
function renderAgents(userSection, others, rules) {
  const head = [
    '# DSH 硬边界（自动生成，来源: ai-configuration + ai-rules/GeneralRules）',
    '',
    '> 本文件由 dsh-plugin-hardbound 自动维护。两个管理标记之间的内容每次同步会重建，请勿手改；',
