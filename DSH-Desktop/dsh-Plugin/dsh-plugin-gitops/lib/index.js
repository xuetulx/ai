...
/**
 * dsh-plugin-gitops / DSH 宿主插件
 * ---------------------------------------------------------
 * 作为 bundle 安装到 profile 后，DSH 启动时自动执行：
 *   1. 启动延迟 3 秒后，后台把 rules/ 目录的 Git 规范同步为 $DSH_HOME/AGENTS.md；
 *   2. 幂等：内容无变化不写文件；失败仅告警，不阻塞 DSH 启动；
 *   3. 提供 /gitops 命令查看同步状态与规则清单。
 *
 * 插件 id 必须与这里导出的 name 一致：plugin-gitops
 */

const core = require('./core');

const name = 'plugin-gitops';
const inject = [];

function runSync(profileName) {
  const r = core.sync({});
  if (!r.ok) {
    console.error('[dsh-plugin-gitops] 规则同步失败: ' + r.error);
    return;
  }
  const verb = r.action === 'written' ? '已写入' : r.action === 'dry-run' ? '(dry-run)' : '无需变更(幂等)';
  console.log(
    '[dsh-plugin-gitops] GitOps 同步' + verb + ': ' + r.rules.length + ' 条规则 -> ' +
