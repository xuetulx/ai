...
/**
 * dsh-plugin-gitops / core
 * ---------------------------------------------------------
 * DSH GitOps 规则注入插件核心逻辑（零外部依赖）。
 *
...
const PLUGIN_ID = 'dsh-plugin-gitops';
...
const BACKUP_NAME = 'AGENTS.md.pre-plugin-gitops.bak';
...
function renderAgents(userSection, others, rules) {
  const head = [
    '# DSH GitOps（自动生成，来源: ai-configuration/02-RULE + 06-MCP）',
    '',
    '> 本文件由 dsh-plugin-gitops 自动维护。两个管理标记之间的内容每次同步会重建，请勿手改；',
