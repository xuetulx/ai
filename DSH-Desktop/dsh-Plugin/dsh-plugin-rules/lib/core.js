...
/**
 * dsh-plugin-rules / core
 * ---------------------------------------------------------
 * DSH 规则注入插件核心逻辑（零外部依赖）。
 *
 ...
 *
 * 本插件职责：把插件内 rules/ 目录下的规则集合并为 $DSH_HOME/AGENTS.md，
 * 使所有会话自动加载这些规则。特性：
 *   - 多插件共存协议：识别任意 `dsh-plugin-*` 管理块，只重建自己的块，
 *     其他插件块原样保留（与 hardbound/gitops/engineering 等可同时挂载）；
 *   - 保留用户区：AGENTS.md 中"所有插件管理块之外"的内容原样保留；
 *   - 首次接管自动备份：AGENTS.md.pre-plugin-rules.bak（幂等，仅一次）；
 *   - 幂等同步：内容哈希无变化时不写文件；
...
const MARK_OPEN = '<!-- ===== dsh-plugin-rules MANAGED SECTION (do not edit) ===== -->';
const MARK_CLOSE = '<!-- ===== end dsh-plugin-rules ===== -->';

/** 首次接管时对既有 AGENTS.md 的备份文件名 */
const BACKUP_NAME = 'AGENTS.md.pre-plugin-rules.bak';
...
/** DSH Harness home：优先 DSH_HOME 环境变量，其次 ~/.dsh（对齐 guard 语义） */
function dshHomeDir() {
...
function scanRules(rulesDir) {
  ...
  const walk = (dir) => {
    ...
    try {
      ...
      console.warn('[dsh-plugin-rules] 读取目录失败: ' + dir + ' - ' + e.message);
      return;
    }
    for (const ent of entries) {
      ...
      try {
        ...
        console.warn('[dsh-plugin-rules] 跳过文件: ' + full + ' - ' + e.message);
      }
    }
  };
  walk(rulesDir);
...
function splitSections(existing) {
  ...
  const user = existing.replace(ANY_BLOCK_RE, (m, id) => {
    if (id !== 'dsh-plugin-rules') others.push({ id, block: m });
    return '';
  }).trim();
  return { user, others };
}

/** 兼容旧 API：返回用户区字符串（仅保留所有插件块之外的内容） */
function splitUserSection(existing) {
  return splitSections(existing).user;
}

/**
 * 生成 AGENTS.md 全文 = 用户区 + 所有插件管理块（按插件 id 稳定排序）。
 * @param {string} userSection 用户保留内容（可为空）
 * @param {Array<{id: string, block: string}>} others 其他插件的管理块（原样保留）
 * @param {Array}  rules       scanRules 的结果
 */
function renderAgents(userSection, others, rules) {
  const head = [
    '# DSH 规则集（自动生成，来源: ai-rules/split）',
    '',
    '> 本文件由 dsh-plugin-rules 自动维护。两个管理标记之间的内容每次同步会重建，请勿手改；',
  ...
  blocks.push({ id: 'dsh-plugin-rules', block: MARK_OPEN + '\n\n' + head + body + '\n\n' + MARK_CLOSE });
...
/**
 * 状态总览。
 * @returns {{ ok, rules, agentsPath, exists, managed, userBytes, totalBytes, hash, backup, error? }}
 */
