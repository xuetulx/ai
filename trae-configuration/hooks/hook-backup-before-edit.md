---
name: hook-backup-before-edit
description: "Hook: 修改文件前强制自动备份。对任何现有文件执行 Edit/Write 覆盖前自动创建 .bak 备份。"
...
## 等效 Rule 表达（无 Hook 支持时降级）

```markdown
[BLOCK] 禁止在未创建 .bak 备份的情况下对任何现有文件
执行 Edit 或 Write（覆盖模式）。
