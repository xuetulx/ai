...
1. `[BLOCK]` AI 生成的源代码/重要产物文件**必须**包含语义化版本号（v1.0、v1.1、v2.0），禁止输出不含版本标识的生产文件
   - 主版本号：重大改动、结构调整
   - 次版本号：功能新增、内容扩充
   - 修订号：文字修正、格式调整、bug 修复
2. `[BLOCK]` 文件名**必须**包含版本号（如 `radar_bk_stats_v1.2.py`），每次修改后递增版本号
3. `[DO]` 规则文件命名格式：`AgentAI_项目文件操作规则_v{主版本}.{次版本}.md`
4. `[DO]` 配套规范同步命名：`AI_RULES_编程参考规范_v{主版本}.{次版本}.md`
5. `[ASK]` 删除旧版本文件前**必须**询问用户确认（保留至少一个历史版本，或归档到 `docs/history/`）
